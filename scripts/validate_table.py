...
if args.file_path:
    ...
    files = sorted(glob.glob(args.file_path), reverse=True)
    if not files:
        raise FileNotFoundError(f"❌ No file found: {args.file_path}")
